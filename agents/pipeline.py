...
from agents.eval import record_agent_success, record_agent_failure, record_fallback, record_pipeline_run
...
async def agent_classifier(state: PaperState) -> PaperState:
    ...
    for attempt in range(2):
        ...
        if result.passed:
            ...
            record_agent_success("classifier", time.time() - t, model="groq")
            return state
    ...
    record_agent_failure("classifier", time.time() - t, error="guardrail_failed", guardrail=True)
    return state
...
async def agent_scorer(state: PaperState) -> PaperState:
    ...
    for attempt in range(2):
        ...
        if result.passed:
            ...
            record_agent_success("scorer", time.time() - t, model="deepseek", score=overall)
            return state
    ...
    record_agent_failure("scorer", time.time() - t, guardrail=True)
    return state
...
async def agent_brief(state: PaperState) -> PaperState:
    ...
    for attempt in range(2):
        ...
        if result.passed:
            ...
            record_agent_success("brief", time.time() - t, model="groq")
            return state
        logger.warning(f"[Agent3] Attempt {attempt+1} failed: {result.errors}")

    state["errors"].append("brief_failed")
    record_agent_failure("brief", time.time() - t, guardrail=True)
    return state
...
async def agent_hype(state: PaperState) -> PaperState:
    ...
    try:
        ...
        record_agent_success("sentinel", time.time() - t, model="sentinel")
        ...
        record_agent_failure("sentinel", time.time() - t, error=str(e))
...
async def analyze_paper(paper: Paper) -> dict:
    ...
    try:
        ...
        record_pipeline_run(success=True, total_latency=time.time() - t)
    except Exception as e:
        record_pipeline_run(success=False, total_latency=time.time() - t)
        raise
